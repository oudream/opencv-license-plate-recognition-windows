...
/* 调整一下中文区域获取算法 */
Rect CCharsSegment::GetChineseRect(const std::vector<Rect>& vecRect,const int& specIndex) {
    cv::Rect rectSpe = vecRect[specIndex];
    cv::Rect nextRect = vecRect[specIndex + 1];

    int chiHeight = nextRect.height;
    float chiWidth = rectSpe.width * 1.1f;

    int speX = rectSpe.x;
    int midSpeX = speX + rectSpe.width / 2;

    int midChiX = midSpeX - rectSpe.width * 1.266f;

    int chiLeftX = midChiX - chiWidth / 2;
    chiLeftX = chiLeftX > 0 ? chiLeftX : 0;

    int chiY = nextRect.y;

    Rect a(chiLeftX, chiY, int(chiWidth), chiHeight);
